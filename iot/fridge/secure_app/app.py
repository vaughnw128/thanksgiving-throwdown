...
def login():
	if request.method == 'POST':
		con, cur = open_server()
		res = cur.execute(f"SELECT * FROM user WHERE username LIKE ? AND password LIKE ?", (request.form['username'], request.form['password']))
		res = res.fetchone()
		con.close()
		if res is None:
...
def change_password():
	if not session.get('user'):
		...
		con, cur = open_server()
		res = cur.execute(f"SELECT * FROM user WHERE username LIKE ? AND password LIKE ?", (session.get('user'), request.form['password']))
		res = res.fetchone()
		if res is None or request.form['new_pass'] != request.form['new_pass_again']:
			con.close()
			return 'Failed to update password<br><a href="/">Home</a>'
		else:
			cur.execute(f"UPDATE user SET password = ? WHERE username LIKE ?", (request.form['new_pass'], session.get('user')))
			con.commit()
			con.close()
